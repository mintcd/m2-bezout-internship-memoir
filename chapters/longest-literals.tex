...
Recall that in the BST-like model, $u$ is the probability of a concatenation node, $v$ is the probability of an alternation node, and $1-u-v$ is the probability of a Kleene star node. For a random regex tree of size $n$, the random variable $Y_n$ denotes the length of the longest literal extracted by Hyperscan, and we write $y_n=\EE[Y_n]$. This chapter studies the asymptotic behavior of $(y_n)$.
...
Consider the recurrence \eqref{eq:longest-literal-recurrence}. Denote $y_n := \EE[Y_n]$ for $n \ge 1$. Taking the expectation of both sides in the recurrence \eqref{eq:longest-literal-recurrence}, we have for $n \ge 1$,
...
\end{equation}
It is possible to employ a transfer theorem from analytic combinatorics to analyze the asymptotic behavior of \eqref{eq:upper-bound} and \eqref{eq:lower-bound} (Appendix \ref{subsec:asymptotics-of-recurrences-that-include-a-sum-of-history}). The lower bound can be tightened by using
...
The sequence defined by \eqref{eq:lower-bound-tighter} is bounded between those defined by \eqref{eq:upper-bound} and \eqref{eq:lower-bound}. It is shown that if the following conditions hold
\begin{equation}
  \label{eq:parameter-conditions}
  u,v \ge 0, \quad u < 1, \quad u+v\le1, \quad 2u+v > 1,
\end{equation}
then both \eqref{eq:upper-bound} and \eqref{eq:lower-bound} grow as a pure power of $n$, without any lower-order factor like the logarithm. In this section, we apply different techniques to prove asymptotic theorems of increasing strength. The strongest theorem is about the exact asymptotics, i.e., $z_n\sim Cn^{\alpha}$ for some constants $C > 0$ and $\alpha > 0$. As a heuristic, assume that $z_n \sim Cn^{\alpha}$ for some constants $C > 0$ and $\alpha > 0$. Substituting this estimate into \eqref{eq:lower-bound-tighter}, we get
\begin{align*}
  Cn^\alpha
...
Note that we indeed have $\alpha>0$, since otherwise $2u+v = 1$. Numerical experiments regarding the ratio $z_n/n^{\alpha}$ are illustrated in Figure \ref{fig:ratios}. Now we state the asymptotic theorems.
...
Let us proceed with Theorem \ref{thm:weaker-asymptotics}. Rewrite the recurrence in \eqref{eq:lower-bound-tighter} as
...
Note that the unique value of $\alpha$ such that $\Lambda(\alpha) = 1$ is the solution to \eqref{eq:alpha-equation}. Indeed, its derivative is
\begin{align*}
...
\begin{proof}[Proof of Theorem \ref{thm:weaker-asymptotics}]
  ...
  Hence, $\dfrac{z_{n+2}}{(n+2)^\beta} < M_n$. Or $(M_n)$ is eventually decreasing. Therefore, $z_n = O(n^\beta)$.

  Similar argument applies for the lower bound, with an adjustment to deal with $z_2$. We have
...
\begin{proof}[Proof of Theorem \ref{thm:theta-asymptotics}]
  ...
  Now we prove the upper bound. Our induction hypothesis is $z_n \le \tilde{K} (n-1)^\alpha$ for $n\ge 2$, for some constant $\tilde{K} > 1$. The base case $z_2=0$ is trivially satisfied. For $n\ge 2$, we have
  \begin{align*}
    nz_{n+2}
...
We now prove the exact asymptotics stated in Theorem \ref{thm:exact-asymptotics}. A natural goal is to remove the maximum operator. If $z_1=z_2=1$, then the sequence would be increasing from the beginning, and induction would allow us to eliminate the maximum operator. However, the initial conditions $z_1=1$ and $z_2=0$ make the sequence $(z_n)$ fluctuate during the first iterations, and the index at which it becomes increasing depends on the parameters $u$ and $v$. An extreme case of a long fluctuation period occurs when $u\to1/2+$ and $v\to0+$. We illustrate this phenomenon and compare the two cases in Figure~\ref{fig:difference-beginning}.
...
Figure~\ref{fig:difference-beginning} compares the consecutive differences of the same recurrence under two close initializations, $z_1=z_2=1$ and $z_1=1,z_2=0$, in the difficult regime $u=0.5000001$ and $v=0.000001$. The second initialization can create a long initial fluctuation, which explains why monotonicity cannot simply be assumed from the beginning.

It turns out that eventual monotonicity is sufficient to remove the maximum operator. Indeed, assume that there exists $N_1$ such that $z_{n+1} \ge z_n$ for all $n \ge N_1$. Since $(z_n)$ is unbounded, there exists $N_2 \ge N_1$ such that for all $n \ge N_2$, we have both $z_{n+1} \ge \max\limits_{1\le i \le N_1} z_i$ and $z_{n+1} \ge z_n$. That is
...
Let us reduce the condition $0<v < \alpha$ in terms of $u$ and $v$. Define
...
For $v>0$, if we impose conditions \eqref{eq:parameter-conditions-stronger}, then the above inequality is satisfied. The case $v\ge \alpha$ with $v>0$ is left open. With $(z_n)$ being eventually increasing, it has a nicer recurrence without the whole history.
...
With the recurrence \eqref{eq:delayed-recurrence}, we can now prove the exact asymptotics of $(z_n)$. We will use a dyadic argument to show that the normalized difference sequence $(\Delta x_n)$ is absolutely summable by analyzing it on the intervals $[2^m, 2^{m+1})$.
        ...
        The stochastic recurrence \eqref{eq:longest-literal-recurrence} looks similar to its deterministic counterpart \eqref{eq:lower-bound-tighter}. With the results that the lower and upper bounds grow as a pure power of $n$, it is plausible to conjecture the same situation for the sequence $(y_n)$ defined by \eqref{eq:expected-longest-literal-recurrence}. Numerical experiments based on Monte-Carlo simulation (see Appendix \ref{subsec:monte-carlo-simulation}) support this conjecture, as shown in Figure \ref{fig:growth-comparison}. For each case of $(u,v)$, we compute the power $\alpha$ using the algorithm expressed in \eqref{eq:mean-preserving-recurrence}, and the constant $C=z_n/n^\alpha$, where $n$ is the number of iterations. More details of the algorithm are given later in this section. The growth power of $y_n$ is strictly larger than the solution to \eqref{eq:alpha-equation}. This suggests that in long-term, $\EE\left[\max(Y_i, Y_j)\right]$ is still more selective than $\max(\EE[Y_i], \EE[Y_j])$, especially when $i$ and $j$ are close to each other.
        ...
        In this section, we present an ongoing attempt towards the weakest asymptotic result $y_n = n^{\alpha^\star + o(1)}$. We adapt the contraction method (e.g., \cite{neininger2004general}). The original idea is as follows.
        \begin{enumerate}
        ...
        The proof of Lemma \ref{lem:limiting-equation} is given in Appendix \ref{subsec:proof-limiting-equation}. Let $\D$ be the space of probability measures on $\mathbb{R}$. Our first goal is to prove that there exists a unique solution $(X^\star,\alpha^\star)$ to \eqref{eq:limiting-equation} in some subspace of $\D$. The question is therefore how to choose this subspace. The fixed-point equation \eqref{eq:limiting-equation} always has an undesired solution $\delta_0$, so the subspace should exclude $\delta_0$. Define the operator $T: \D \to \D$ induced by the right-hand side of \eqref{eq:limiting-equation} as
        \begin{equation}
          \label{eq:operator-T}
        ...
        A promising approach to find the true $\alpha$ is to let the distribution and its mean-preserving operator evolve together. By Proposition \ref{prop:f}, the function $\alpha: \mathcal{D}(1) \to (0,1]$ such that $\EE_{X\sim\mu}[T_{\alpha(\mu)}(X)] = 1$ is well-defined. We can then define the mean-preserving recurrence as follows. The evolution of $\alpha$ and the 2-Wasserstein distance between consecutive distributions following the recurrence is shown in Figure \ref{fig:mean-preserving-recurrence-evolution}. Fluctuations in the final iterations are likely a side effect of the Monte-Carlo simulation, since they become more stable when the number of samples is increased.
...
Since the contraction argument may not be applicable to proving the uniqueness of the fixed point, we decompose the problem to proving existence and then uniqueness. Existence is proved using Tychonoff fixed-point theorem.
...
In the following, we will show that the subset $\C$ can be restricted to the set of distributions with uniformly bounded second moment.
