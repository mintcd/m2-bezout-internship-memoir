...
The architecture of Hyperscan includes two phases: compilation and execution. In the compilation phase, a set of regular expressions is converted into an internal database. The database is used in the execution phase to match the regular expressions against a stream of data. Hyperscan supports two modes of operation: block mode and stream mode. In block mode, the engine processes one block of data at a time. In stream mode, it maintains state across blocks, allowing continuous matching over a data stream. We focus on block mode.
...
The components are added one by one through \href{https://github.com/intel/hyperscan/blob/master/src/nfagraph/ng.cpp#L205}{\texttt{addComponent}}. Each component is then passed to the Violet subengine through the \href{https://github.com/intel/hyperscan/blob/master/src/nfagraph/ng_violet.h#L49}{\texttt{doViolet}} function, which calls \href{https://github.com/intel/hyperscan/blob/master/src/nfagraph/ng_violet.cpp#L2989}{\texttt{doInitialVioletTransform}} and \href{https://github.com/intel/hyperscan/blob/master/src/rose/rose_build_impl.h#L507}{\texttt{RoseBuild.addRose}}.
...
There are at least four SIMD-based string matchers in Hyperscan: Noodle is used for a single literal, Teddy \cite{qiu2021teddy} or Harry \cite{xu2023harry} for 2 to 300 literals, and FDR for more than 300 literals. The last three matchers are prefilter-based: they first filter the text to find candidate matches, and then verify them. FDR is our main candidate for analysis. It extends the classical Shift-Or \cite{baeza1992new} algorithm to match multiple patterns at once. In this section, we review the Shift-Or algorithm and the two FDR phases, compilation and execution.
...
\begin{definition}
  ...
  where $\emptyset \neq p_i \subseteq \Sigma$ for all $i \in [1, n]$. A position equal to $\Sigma$ is usually referred to as a \textit{don't care} position. A string $T$ over $\Sigma$ matches the indeterminate string $\bm{P}$ if and only if $|T| = n$ and $T[i] \in \bm{p}_i$ for all $i \in [1, n]$.
\end{definition}
...
The implementation chooses $\alpha=1.05$ and $\beta=3$. If one uses the same cost function over the set of all possible assignments, the optimal assignment can be achieved by partitioning $\tilde{\bm{P}}$. Thus FDR effectively reduces a combinatorial optimization problem to a dynamic programming problem. We state this fact as follows.
...
\begin{proof}
  ...
  We prove the other direction. Let $\sigma$ be an assignment function that minimizes $g$, and suppose that its 8 buckets have sizes $n_0, n_1, \dots, n_7$ with $\sum_{i=0}^7 n_i = m$. Let $L_j = \min_{p \in \sigma^{-1(j)}} |p|$ be the minimum pattern length in bucket $j$. Without loss of generality, we can assume that $L_0 \le L_1 \le \dots \le L_7$. Construct a new, contiguous partition of the sorted array $\tilde{\bm{P}}$ using these same sizes $n_0, \dots, n_7$. The boundaries are $b_0 = 0$ and $b_{j+1} = b_j + n_j$. Let $L'_j = \left|p_{b_j}\right|$ be the minimum pattern length in bucket $j$ under the partition. Then $L'_j \ge L_j$ for all $j$, and therefore
  $$\frac{n_j^\alpha}{(L'_j)^\beta} \le \frac{n_j^\alpha}{L_j^\beta}, \forall j\in\{0,\ldots,7\}.$$
  This yields a new assignment function $\sigma'$ such that $g(\sigma') \le g(\sigma)$. Since $\sigma'$ is a partition of $\tilde{\bm{P}}$, we have $\min f(b_1,\ldots, b_6) \le g(\sigma')$. This completes the proof.
\end{proof}
...
The last step is to build a hash table for the patterns. Let $h$ be a hash function. Investigating it in the original implementation is left to future work. An array \texttt{arr} is built such that $\texttt{arr}[h(p)]$ stores information about the bucket containing the pattern $p$.
...
We now match against the text $babc$. Since the text has only $4$ characters, there is only one iteration. We detail the scan character by character, considering the right-most $4$ bytes. The new bit set to $1$ after each character is scanned is underlined.
...
The algorithm reports a match in bucket $B_0$ ending at position $1$, a match in bucket $B_1$ ending at position $2$, and a match in bucket $B_0$ ending at position $3$. Since the pattern lengths in each bucket are known, we can retrieve the beginning positions of these potential matches and verify them by exact string comparison. In this case, all three potential matches are indeed matches. The pseudocode is given in Algorithm~\ref{alg:compilation} and Algorithm~\ref{alg:execution}.
...
for \textit{every} character $x$. This can be done more efficiently, since there are only $2^{s-8}$ such super-characters. The corrected compilation pseudocode is given in Algorithm~\ref{alg:compilation-super}. The changes and additions are highlighted in \textcolor{gustave}{blue}.
...
Another parameter is the \textit{stride}. For example, if the stride is $2$, then we traverse $j$ at $0$, $2$, $4$, and $6$. Each step of $j$ requires a query to the mask dictionary, which is not constant-time in the implementation. The trade-off is that there are more positives to verify.
...
Since Hyperscan extracts and matches literals before executing the sub-FAs, it is worth considering the average length of the longest literal that Hyperscan attempts to match. Let this quantity be denoted by a random variable $Y_n$ for a random regular expression tree of size $n$. If the root is a concatenation, then literals from the left and right subtrees can be concatenated, so the longest literal is the sum of the two subtree contributions. If the root is an alternation, then a string follows one branch or the other, so the longest literal is the maximum of the two subtree contributions. If the root is a Kleene star, the language contains the empty string; in the model used here, Hyperscan does not extract a literal through that root, and the contribution is $0$.
