...
Suppose that the regex trees are generated according to a BST-like distribution. The average length of the longest literals of an $n$-node tree satisfies the recurrence
\begin{equation}
...
Superscripts denote mutually independent copies. The variable $U_n$ is uniformly distributed on $\{1, \ldots, n\}$. The variables $I$ and $J$ are Bernoulli distributed with expectations $u$ and $v$ respectively, such that $I + J \le 1$. We reuse the same variables $Y_{U_n}^{(1)}, Y_{n+1-U_n}^{(2)}$ and $U_n$ across both terms, since the branches are not taken at the same time. The variables $I$ and $J$ are also reused, since at every new iteration, we sample a new subtree independently.

Details analysis for the asymptotic behavior of $\EE[Y_n]$ is given in the next chapter.