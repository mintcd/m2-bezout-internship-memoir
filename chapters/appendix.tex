...
\begin{proof}
  ...
  Equating the two sides and rearrange, we obtain a first-order linear differential equation
  ...
  Furthermore, $\lim_{x\to 0} \frac{(1-x)^K e^{Kx} - 1}{x^2} = -\frac{K}{2}$, confirming the right-hand integrand is continuous. Moving $-z_1 x^{-2}$ to the left side and integrating from $0$ to $x$ via the Fundamental Theorem of Calculus yields
  ...
  We must ensure singularity cancellation does not occur. For any $t \in (0, 1)$, the inequality $\ln(1-t) < -t$ implies $K\ln(1-t) + Kt < 0$. Exponentiating yields $(1-t)^K e^{Kt} < 1$, which means the integrand is strictly negative. Consequently, the integral evaluates to a strictly negative value, making $1 - \int_0^1 \frac{(1-t)^K e^{Kt} - 1}{t^2} \, \mathrm{d}t > 1$. Because $z_1, z_2 \ge 0$ and are not both zero, it strictly follows that $A \ge z_2 + z_1 > 0$. Since $K > 0$, we apply the Theorem \ref{thm:sim-transfer}. Mapping the $(1-x)^{-K}$ singularity to the coefficients $z_n = [x^n]Z(x)$ yields$$z_n \sim \frac{A e^{-K}}{\Gamma(K)} n^{K-1}.$$Defining $C = \frac{A e^{-K}}{\Gamma(K)}$, we have $C > 0$, completing the proof.
\end{proof}
...
\begin{proof}
  Using $M_0$ from Proposition \ref{prop:uniform-bounded-second-moment} and choose $M=\max(M_0, 1)$, then the set $\D_2(1, M)$ is nonempty since it contains $\delta_1$.
  ...
  For compactness, we need to show that $\D_2(1, M)$ is tight (Definition \ref{def:tight}) and closed. For tightness, we use Chebyshev's inequality. For any $\mu \in \D_2(1, M)$ and any $R > 0$
